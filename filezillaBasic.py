...
class Filezilla:
    ...
    def up_to_server(self, imgfile, id, mode):
        ...
        if mode == 1:
            self.ftp.cwd("/Driver images")  # Folder path
        elif mode == 2:
            self.ftp.cwd("/Plate num")  # Folder path
        elif mode == 3:
            self.ftp.cwd("/Webcam image")  # Folder path
        ...
        for f in filelist:
            if imgfile in f:
                with open(imgfile, "wb") as file:
                    # Command for Uploading the file "STOR filename"
                    self.ftp.retrbinary(f"RETR {imgfile}", file.write)
                    f = 1
